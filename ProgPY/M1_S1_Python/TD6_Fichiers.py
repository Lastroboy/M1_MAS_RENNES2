...
# ou
def import_data(fic):
    with open(fic, "r", encoding="utf-8") as fic_in:
        contenu = json.load(fic_in)
    return contenu
...
def export_json_to_csv(restaurants, nom_fichier_csv):
    with open(nom_fichier_csv, "w", encoding="utf-8", newline="\n") as fp:
        csvfp = csv.DictWriter(fp, fieldnames=restaurants[0].keys(),delimiter=";")
        csvfp.writeheader()
        for r in restaurants:
            csvfp.writerow(r)

def export_restautants(restaurants, fic):
    with open(fic, "w", newline="\n", encoding="utf-8") as fic_out:
        csv_writer = csv.DictWriter(fic_out, fieldnames=restaurants[0].keys(), delimiter=";")
        csv_writer.writeheader()  # Écrire les en-têtes
        csv_writer.writerows(restaurants)  # Écrire les données
